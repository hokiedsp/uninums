...
function normalSpaces(s) {
  return s
    ? s.toString().replace(Zs_and_friends, " ").replace(line_terminators, "\n")
    : s;
}
...
function parseUniInt(s, radix) {
  return parseInt(
    s && typeof s != "number" ? normalDigits(normalSpaces(s.toString())) : s,
    radix
  );
}
...
function sortNumeric(array) {
  return array.sort(function (a, b) {
    var va = parseUniFloat(a),
      vb = parseUniFloat(b);
    return isNaN(va) ? -1 : isNaN(vb) ? 1 : va < vb ? -1 : va == vb ? 0 : 1;
  });
}

export { normalSpaces, normalDigits, parseUniInt, parseUniFloat, sortNumeric };
export default { normalSpaces, normalDigits, parseUniInt, parseUniFloat, sortNumeric };
